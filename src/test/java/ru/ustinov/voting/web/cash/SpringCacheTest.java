...
public class SpringCacheTest extends AbstractControllerTest {
    ...
    public void cacheEvictWhenUpdateRestaurant() throws Exception {
        ...
        final List<Restaurant> resBeforeUpdateUser = cacheManager.getCache("restaurants").get("getWithDishesToday", List.class);
        WITH_DISHES_MATCHER.assertMatch(resBeforeUpdateUser, Stream.of(RESTAURANT_CI, RESTAURANT_HARBIN).sorted(comparing(NamedEntity::getName)).toList());
        ...
        perform(MockMvcRequestBuilders.put(REST_URL_ADMIN + RESTAURAUNT_HARBIN_ID)
                ...
                .andDo(print());
        getRestaurantsWithDishes();
        final List<Restaurant> resAfteUpdateUser = cacheManager.getCache("restaurants").get("getWithDishesToday", List.class);
        WITH_DISHES_MATCHER.assertMatch(resAfteUpdateUser, Stream.of(RESTAURANT_CI, getUpdated()).sorted(comparing(NamedEntity::getName)).toList());
    }
    ...
    public void cacheEvictWhenUpdateDish() throws Exception {
        ...
        perform(MockMvcRequestBuilders.put(DishController.REST_URL + '/' + RESTAURAUNT_HARBIN_ID + '/'+ DISH_ID)
                ...
                .andDo(print());
        getRestaurantsWithDishes();
        final List<Restaurant> resAftereUpdate = cacheManager.getCache("restaurants").get("getWithDishesToday", List.class);
